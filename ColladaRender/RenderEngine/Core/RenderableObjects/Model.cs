...
namespace ColladaRender.RenderEngine.Core.RenderableObjects
{
    ...
    {
        private HashSet<Vertex> _vertexSet = new();
        ...
        public Model(List<Vertex> toLoad)
        {
            ...
            foreach (var curVertex in toLoad)
            {
                ...
                else
                {
                    ...
                    processedIndices.Add(processedVertexList[curVertex]);
                }
            }

            //Strip the values from each vertex
            foreach (var curVertex in _vertexSet)
            {
